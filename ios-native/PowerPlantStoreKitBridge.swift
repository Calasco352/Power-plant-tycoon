...
final class PowerPlantStoreKitBridge: NSObject, WKScriptMessageHandler {
    ...
    static let autoGenerateProductID = "com.calasco352.powerplanttycoon.autogenerate"
    ...
    func userContentController(_ userContentController: WKUserContentController,
        ...
        Task { @MainActor in
            ...
            case "products":
                await loadProducts()
            case "purchase":
                if let productID { await purchase(productID: productID) }
            case "restore":
                await restorePurchases()
            default:
                send([
                    "status": "error",
                    "message": "Unknown StoreKit action."
                ])
            }
        }
    }
    ...
    private func loadProducts() async {
        do {
            let fetched = try await Product.products(for: [Self.autoGenerateProductID])
            products = Dictionary(uniqueKeysWithValues: fetched.map { ($0.id, $0) })
            let prices = Dictionary(uniqueKeysWithValues: fetched.map { ($0.id, $0.displayPrice) })
            send([
                "status": "products",
                "prices": prices
            ])
        ...
        do {
            ...
            if let cached = products[productID] {
                ...
                guard let first = fetched.first else {
                    send(["status": "error", "message": "Purchase item is not available."])
                    return
                }
            ...
            case .success(let verification):
                ...
                await sendEntitlements(status: "purchased", productID: transaction.productID)
            case .pending:
                send(["status": "pending", "productID": productID])
            case .userCancelled:
                send(["status": "cancelled", "productID": productID])
            @unknown default:
                send(["status": "error", "message": "Unknown App Store purchase result."])
            }
        } catch {
            send([
                "status": "error",
    ...
    private func restorePurchases() async {
        do {
            ...
            await sendEntitlements(status: "restored")
        } catch {
            send([
                "status": "error",
    ...
    private func sendEntitlements(status: String, productID: String? = nil) async {
    ...
    private func verified<T>(_ result: VerificationResult<T>) throws -> T {
        ...
        case .verified(let safe):
            return safe
        case .unverified:
            throw StoreBridgeError.failedVerification
        }
    }
