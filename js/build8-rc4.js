/* POWER PLANT TYCOON — BUILD 8 RC4 RELEASE VERIFICATION */
(function(){
  if(window.__pptBuild8RC4)return;
  window.__pptBuild8RC4=true;
  document.body.classList.add("ppt8-rc4");
  ...
  if(typeof offlineEfficiency==="function"){
    offlineEfficiency=function(){
      /* Match the App Store product promise while keeping online play stronger.
         Free players: 20% base offline efficiency.
         Offline Operations License: 35% base offline efficiency.
         A small earned legacy bonus may add up to +2 percentage points. */
      const legacy=Math.min(.02,n(g?.legacy?.offline)*.001);
      const licensed=!!g?.offlineOperationsUnlocked;
      const base=licensed?.35:.20;
      return Math.min(licensed?.37:.22,base+legacy);
    };
  }

  /* RC3 AUTO SELL SAFETY
     ...
     fleet was actually producing. RC3 always keeps a visible production buffer:
  ...
  function injectStyles(){
    if(document.getElementById("ppt8RC4Styles"))return;
  ...
  function ensureHomeVerification(){
    ...
    if(gen&&!document.getElementById("ppt8GenerateHelp")){
      ...
      h.textContent="Your fleet generates power automatically. GENERATE POWER adds an extra production pulse.";
  ...
  function renderHomeVerification(){
    ...
    if(fs){
      ...
      const last=window.__ppt8AutoSellLast||{}; const sold=n(last.amount)>0?' • Auto sold '+fmtEnergy(last.amount):''; fs.innerHTML='<span><b>'+a.rows.length+' / '+total+' plants online</b> • +'+fmtEnergy(a.hud)+'/sec to storage'+sold+'</span><span class="'+(a.pass?'ok':'warn')+'">'+(a.pass?'✓ ONLINE':'⚠ CHECK OUTPUT')+'</span>';
    }
    ...
    if(as){
      ...
      as.innerHTML=g.autoSell
        ?'<strong>⚡ AUTO SELL ON</strong> • '+mode+' mode • Maintains a visible reserve so Stored Power stays available.'
  ...
  function renderFleetAuditCard(){
    ...
    card.innerHTML='<div class="ppt8-audit-head"><div><small>FLEET STATUS</small><br><b>⚡ Production Breakdown</b></div><span class="'+(a.pass?'ppt8-pass':'ppt8-warn')+'">'+(a.pass?'✓ MATCHED':'⚠ REVIEW')+'</span></div><div class="small">'+a.rows.length+' of '+total+' plant technologies online. Every unlocked plant below is included in the total Output shown at the top.</div>'+rows+'<div class="ppt8-total"><span>Fleet total</span><strong>'+fmtPower(a.sum)+'</strong></div><div class="ppt8-total"><span>Displayed output</span><strong>'+fmtPower(a.hud)+'</strong></div>';
  }
  ...
  function correctStoreCopy(){
    document.querySelectorAll(".b6-product").forEach(card=>{
      ...
      else if(title==="Offline Operations License")p.textContent="35% base offline efficiency vs 20% free, with a strict offline-production safety cap so online play remains more rewarding.";
    });
  ...
  console.info("Power Plant Tycoon • Build 8 RC4 release verification active");
})();
