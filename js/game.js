...
function upgradeTap(){const c=tapUpgradeCost();if(g.cash<c){toast("Need "+money(c));return}g.cash-=c;g.tapLevel++;addXP(4);addLog("Manual generator upgraded to Level "+(g.tapLevel+1)+".");saveGame();render()}function buildPlant(id){playPremiumSfx("upgrade");feedback("big");const p=PLANTS.find(x=>x.id===id),s=g.plants[id],c=plantCost(p);if(g.cash<c){toast("Need "+money(c));return}g.cash-=c;if(!s.unlocked){s.unlocked=true;s.level=1;s.condition=100;addXP(20);addLog(p.name+" commissioned at Riverbend.");toast(p.name+" ONLINE!")}else{s.level++;s.condition=Math.min(100,s.condition+8);addXP(10);addLog(p.name+" upgraded to Level "+s.level+".")}saveGame();render()}
...
function prestige(){if(g.lifetimeCash<50000){toast("Earn $50,000 lifetime cash first.");return}askConfirm("Prestige Company","Reset cash, plants and regions for +10% permanent production?",()=>{const p=g.prestige+1,a=g.achievements,settings=g.settings;g=defaultGame();g.prestige=p;g.achievements=a;g.settings=settings;g.tutorialStep=5;g.log=["Company prestiged."];saveGame();render();feedback("big")})}function resetGame(){askConfirm("Erase Save?","This permanently resets your local Power Plant Tycoon progress.",()=>{localStorage.removeItem("PPT_V5");g=defaultGame();saveGame();render();toast("Save reset")})}
